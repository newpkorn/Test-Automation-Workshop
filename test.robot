*** Settings ***
Library     SeleniumLibrary
Resource    variables.resource
Library     env_keywords.py
Library     String


*** Test Cases ***
Test-Order Completed
    Open Web Browser
    Login
    Order Details


*** Keywords ***
Open Web Browser
    Open Browser
    ...    https://login-customer.cpall-allmember.net/login?
    ...    chrome
    Maximize Browser Window

Login
    ${u} =    Get Env Variable    username
    ${p} =    Get Env Variable    password
    Wait Until Element Is Visible    name=email    10s
    Input Text    name=email    ${u}
    Input Password    name=password    ${p}
    Click Element    ${LOCATOR_BTN_LOGIN}

Order Details
    Sleep    5s
    Go To
    ...    https://www.allonline.7eleven.co.th/account/order-history/order-details/?orderId=55301380&checkSum=a8ee1b9dcd5765f8a3ef77b95b4a736b
    Wait Until Page Contains    รายละเอียดการสั่งซื้อ    20s

    Element Text Should Be
    ...    ${LOCATOR_CHK_ORDER_NUMBER}
    ...    หมายเลขสั่งซื้อ #${order_number}

    # check order date
    Element Text Should Be
    ...    ${LOCATOR_CHK_ORDER_DATE}
    ...    วันที่สั่งซื้อ ${order_date}

    # check payment method
    Element Text Should Be
    ...    ${LOCATOR_PAYMENT_TYPE}
    ...    ชำระเงินสด ที่ร้านเซเว่นอีเลฟเว่น (7-11)

    # check delivery address
    Wait Until Element Is Visible    xpath=//div[@class='address']//span[contains(@class, 'address ellipsis-full')]
    ${address_text} =    SeleniumLibrary.Get Text
    ...    xpath=//div[@class='address']//span[contains(@class, 'address ellipsis-full')]
    ${address_text_string} =    BuiltIn.Set Variable    ${address_text}
    ${cleaned_address_text} =    BuiltIn.Evaluate
    ...    "str(${address_text_string}).replace('\\n', ' ').replace('
    ...    ', ' ')"

    Log    ${cleaned_address_text}
    Should Contain    ${cleaned_address_text}    ชญานิศา วงศ์ภักดี
    Should Contain    ${cleaned_address_text}    เบอร์โทรติดต่อ : 0895678912
    Should Contain    ${cleaned_address_text}    45/89 พระโขนง, คลองเตย คลองเตย กรุงเทพมหานคร 10110

    # check product details
    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[1]/div[1]/div[1]/span[2]
    ...    ${TOTAL_PRICE}
    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[1]/div[1]/div[2]/span[2]
    ...    ${SHIPPING_FEE}

    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[5]/div/div/div/div[2]/div[4]
    ...    ${NET_TOTAL}
    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[1]/div[2]/div[2]/span
    ...    ${ALL_MEMBER_POINTS}

    # Order Details
    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[4]/div[2]/div[2]
    ...    ${PRODUCT_ITEM_1}
    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[4]/div[3]/div[1]/span[2]
    ...    ${PRODUCT_QTY}
    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[4]/div[3]/div[2]/span[2]
    ...    ${PRODUCT_PRICE_ITEM_1}

    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[7]/div[2]/div[2]
    ...    ${PRODUCT_ITEM_2}
    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[7]/div[3]/div[1]/span[2]
    ...    ${PRODUCT_QTY}
    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[7]/div[3]/div[2]/span[2]
    ...    ${PRODUCT_PRICE_ITEM_2}

    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[10]/div[2]/div[2]
    ...    ${PRODUCT_ITEM_3}
    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[10]/div[3]/div[1]/span[2]
    ...    ${PRODUCT_QTY}
    Element Text Should Be
    ...    xpath=//*[@id="page"]/div[2]/div[2]/div[2]/div[6]/div/div[10]/div[3]/div[2]/span[2]
    ...    ${PRODUCT_PRICE_ITEM_3}
